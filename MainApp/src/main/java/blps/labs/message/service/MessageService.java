...
public class MessageService {
    ...
    public void sendMessageWhenReviewAdd(Review review) {
        ...
        try {
            ...
            rabbitMQSender.send(new AddReviewMessage(review.getId(), user.getEmail(), user.getUsername(), review.getCar().getCarModel()));
    ...
    public void sendMessageWhenReviewChecked(Long id, String message, Boolean approved) {
        ...
        try {
            ...
            rabbitMQSender.send(new CheckReviewMessage(review.getId(), user.getEmail(), user.getUsername(), review.getCar().getCarModel(), approved, message));
        } catch (UsernameNotFoundException ignored) {
            log.warn("Review belongs to not auth user: message is not sent");
        }
    }
